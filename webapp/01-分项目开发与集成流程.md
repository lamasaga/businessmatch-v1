# 商域 BizSim Edu · 分项目开发与集成流程

> **文档定位**：在 [`00-项目现状与发展设计.md`](./00-项目现状与发展设计.md)（战略与现状）与 [`README.md`](./README.md)（工程使用）之间，给出**可执行的后续开发流程**：先组织者控制端 → 基础商业模拟赛制内容 → 与现有学生端结合；并说明**多项目并行、单库共享、AI 协作**的操作方法。
>
> **读者**：产品、架构、独立负责子模块的开发者、使用 Cursor 协作的本人  
> **关联文档**：[`00-项目现状与发展设计.md`](./00-项目现状与发展设计.md) §1.4 / §6.3～6.4、[`../商业模拟教育平台/02-技术栈与项目结构.md`](../商业模拟教育平台/02-技术栈与项目结构.md)、[`../商业模拟教育平台/06-规模化技术栈与数据治理.md`](../商业模拟教育平台/06-规模化技术栈与数据治理.md)、[`../商赛界面展示/00-解耦声明式商业模拟框架总览.md`](../商赛界面展示/00-解耦声明式商业模拟框架总览.md)  
> **最后更新**：2026-05-19
...
　　短期不必一次性迁到 `platform/`，但**命名与边界**建议按此目标渐进（对齐 [`../商业模拟教育平台/02-技术栈与项目结构.md`](../商业模拟教育平台/02-技术栈与项目结构.md)）：
...
| 项 | 参考 |
|----|------|
| Practice 模式 + AI 对手 | [`../商业模拟教育平台/05-AI深度融合设计.md`](../商业模拟教育平台/05-AI深度融合设计.md) |
| 第 2 种赛制 | 复制「配置包 + UI 切片」，不复制后端 |
| 教师/机构看板 | 可并入组织者端 Tab（见 `00-` §6.4） |
| 24 个月全路线 | [`../商业模拟教育平台/04-分阶段实施路线图.md`](../商业模拟教育平台/04-分阶段实施路线图.md) |
...
| 文档 | 在流程中的用法 |
|------|----------------|
| [`00-项目现状与发展设计.md`](./00-项目现状与发展设计.md) | Phase 验收标准（§七 Phase A～C） |
| [`../商业模拟教育平台/04-`](../商业模拟教育平台/04-分阶段实施路线图.md) | 24 个月节奏；**当前仅做 P1 子集** |
| [`../商赛界面展示/00-解耦声明式…`](../商赛界面展示/00-解耦声明式商业模拟框架总览.md) | Phase 2 引擎与配置包蓝本 |
| [`../商业模拟教育平台/05-`](../商业模拟教育平台/05-AI深度融合设计.md) | Phase 3 之后再上 Athena/Demia/Rival |
...
| W3 | 学生端隐藏组织者路由；官方赛 → `xp_events(official)` | 双端联调通过 00- §七 Phase A |
...
| 文档 | 用途 |
|------|------|
| [`README.md`](./README.md) | 安装、启动、API、目录结构 |
| [`00-项目现状与发展设计.md`](./00-项目现状与发展设计.md) | 战略、现状矩阵、Phase A～C 验收 |
| 本文档 | 分项目开发顺序、单库规则、AI 协作 |
| [`../商业模拟教育平台/README.md`](../商业模拟教育平台/README.md) | 五域与 24 个月规划入口 |

### 9.2 维护说明

- 新增 `organizer-frontend` 或 `contracts/` 时，更新本文 §三目录树与 §9 索引。  
- Phase 0～3 验收标准变更时，同步更新 §四、§七，并回写 `00-` §七 若存在冲突。  
