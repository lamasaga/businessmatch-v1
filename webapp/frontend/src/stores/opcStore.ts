...
export const useOPCStore = create<OPCState>((set) => ({
  ...
  fetchCompany: async (id: number) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/opc/companies/${id}`);
      const json = await res.json();
      if (json.success || json.code === 200 || json.code === 0) {
  ...
  fetchEmployees: async (companyId: number) => {
    try {
      const res = await fetch(`${API_BASE}/opc/companies/${companyId}/employees`);
      const json = await res.json();
      if (json.success || json.code === 200 || json.code === 0) set({ employees: json.data });
    } catch (e: any) {
      set({ error: e.message });
    }
  },
  ...
  fetchTasks: async (companyId: number) => {
    try {
      const res = await fetch(`${API_BASE}/opc/companies/${companyId}/tasks`);
      const json = await res.json();
      if (json.success || json.code === 200 || json.code === 0) set({ tasks: json.data });
    } catch (e: any) {
      set({ error: e.message });
    }
  },
  ...
  updateTask: async (taskId, data) => {
    const res = await fetch(`${API_BASE}/opc/tasks/${taskId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data),
    });
  ...
  createCompany: async (name, description) => {
    const res = await fetch(`${API_BASE}/opc/companies`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
