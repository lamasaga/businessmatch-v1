...
　　**2026-05 现状**：组织者端已独立（`organizer-frontend`）；后端已拆 **Arena / Cybercore / Career** 域与 `games/trading` 引擎，首份赛制包 `backend/content/game-configs/trading-v1.yaml` 可加载；`match_kind`（official/practice）与 `xp_events` 已入库；`POST /practice/trading/start` 可创建练习局。仍待完成：

1. **学生端接练习场**（`/games` 入口 → 调 practice API → 跳转 trading 对局页）  
2. **Career 前端读账本**（`/career` 对接 `xp_events` 或聚合 API，替代 mock）  
3. **`contracts/`** OpenAPI 与域事件文档（子项目/AI 协作的单一真相）
...
| **内容层** | `商赛界面展示/`、`商赛主题与真题库/` → `webapp/backend/content/game-configs/*.yaml` | 在多个前端里各写一套规则逻辑 |
...
│   ├── organizer-frontend/          # ★ 组织者端（Phase 1，:5174）
...
| 任务 | 产出 | 状态（2026-05-20） |
|------|------|-------------------|
| SQLite → PostgreSQL + Alembic | 迁移脚本、Docker 一键起库 | 🔴 仍为 SQLite；`migrate_schema.py` 补列旧库 |
| `match_kind`、`xp_events` | 表字段 + `domains/career` | ✅ 已落地；正式赛结束 `settle_match_rewards` |
| Arena 域 + `trading-v1.yaml` | `domains/arena`、`cybercore`、`games/trading` | ✅ 首版已跑通 official |
| `practice` API | `api/practice.py` | ✅ 后端可建练习局；前端未接 |
| 建立 `contracts/` | OpenAPI + 域事件 | 🔴 待建 |

**验收**：同一学生在 DB 中能查到「1 场 official + 1 场 practice」两条 `xp_events`（权重可不同）——**后端能力已具备，需联调学生端练习入口完成端到端演示**。
...
| 线 | 目录 | 交付 |
|----|------|------|
| **A. 赛制包（内容）** | `webapp/backend/content/game-configs/trading-v1.yaml` ✅ + [`./商赛界面展示/00-框架配置Schema与接口契约.md`](../商赛界面展示/00-框架配置Schema与接口契约.md) | 回合数、商品、手续费、阶段机——教研可改 YAML |
| **B. 引擎（代码）** | `backend/app/domains/cybercore` + `backend/app/games/<engine>/` | ✅ trading 已抽离；`packages/cybercore-engine` 仍为远期可选 |
