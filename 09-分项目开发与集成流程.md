...
- ✅ **体验营营团 P1**：`teaching_groups` API + 双端 `/camp`
- 🟡 **赛事工坊 MVP**：`domains/sandbox` + 前端 `/sandbox`（内存会话；待 admin 门控与迁组织者端）
...
　　四赛制并行、业务匣子挂载、演示环境端口与 TechVenture 双轨说明见 **[`10-四赛制匣子开发与集成指导`](./10-四赛制匣子开发与集成指导.md)**（与本文 §三 目录树互补，不重复竖切流程）。
...
| **体验营 P1** | `02-` §5.3 + `08-` AI_DEFAULT + `api/teaching_groups.py` + 双端 `/camp` | ADR-009 · `arena/models/teaching_group.py` | `seasons` 表 · 改 `match_kind` 枚举 |
...
| [`10-`](./10-四赛制匣子开发与集成指导.md) | 四赛制匣子边界、Monorepo 目标、`game-ui`、双轨 TechVenture、云演示端口 |
