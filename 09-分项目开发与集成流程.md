...
　　**2026-05 现状**：组织者端已独立；Arena / Cybercore / Career 域与 `games/trading` 引擎已分层；赛制包 **`trading-v1`（回合）** 与 **`trading-v2-rts`（浮生记即时）** 并存；RTS 采用 **调度器单写 tick + WebSocket 推送 + HTTP 只读**；`POST /practice/trading/start` 默认 v2。仍待完成：
