...
　　好的开场 = AI 不跑偏 + 减少返工。每次开新 Cursor 会话，建议用 **2～3 句话**主动声明范围（规则是兜底，声明是导航）：
...
5. **推送前**：大型更新须 [docs-align-before-push.mdc](./.cursor/rules/docs-align-before-push.mdc) 对齐 `00～09` / `08-`。
...
| 任务类型 | 推荐 attach | 禁止 |
|----------|-------------|------|
| 赛制/Arena 开发 | `02-` **§5.0** + `03-` + `08-` §2.7 + `ARCHITECTURE.md` + 相关 YAML | 改 Career 表 |
| 生涯 / Career Hub | [06-生涯模式](./06-生涯模式-大循环家园与资源经济.md) + [career/DESIGN.md](./webapp/backend/app/domains/career/DESIGN.md) + `08-` §三 Career | 改 Arena 赛制表、跳过 career 域表 |
| 前端页面 | `08-` §2.6 + §2.2 功能矩阵 | 新增后端路由 |
| 商赛 UI / 美术资源 | [inspire/76-](./inspire/76-商赛美术资源嵌入与技术选型建议.md) + `02-` + `inspire/b/00-` + 目标 `game-configs` YAML | 改引擎结算、无 manifest 硬编码路径 |
| Career/生涯 | `01-` §三 + `03-` §三 career 行 + `08-` §2.7 | 改 `games/trading` 结算 |
| OPC | `05-` + `03-` §八 + `OPC/02-` | 抢 Phase A 优先级 |
| 智能体 | `03-` §八 全节 + `05-` §六 | 跳过规则层直接上 LLM |
| 全局架构 | `00-` + `03-` + `09-` + `docs/decisions/README.md` | 单次改多个域 |
| 架构争议/推翻旧方案 | 相关 `docs/decisions/00N-*.md` | 只改代码不写 ADR |
