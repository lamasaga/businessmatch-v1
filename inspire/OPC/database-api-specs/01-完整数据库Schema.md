...
> **文档定位**：OPC 模块的 PostgreSQL 数据库完整定义，包含 DDL、约束、索引、触发器、注释以及对应的 SQLAlchemy ORM 模型。
>
...
CREATE TABLE one_companies (
    ...
    slug VARCHAR(100) UNIQUE NOT NULL,              -- 子域名标识，如 slug.OPC.edu
...
def init_db(database_url: str = "postgresql://user:pass@localhost/opc"):
    """初始化数据库，创建所有表"""
...
if __name__ == "__main__":
    # 开发环境使用 SQLite
    engine = init_db("sqlite:///opc_dev.db")
...
# 创建迁移脚本
alembic revision --autogenerate -m "init_OPC_schema"
