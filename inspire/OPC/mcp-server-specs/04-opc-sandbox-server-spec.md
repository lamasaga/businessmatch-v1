# OPC-sandbox MCP Server 规格文档

> **文档定位**：定义 OPC 平台安全代码执行环境的 MCP Server 完整规格，包含沙箱安全架构、资源限制、文件系统隔离、网络白名单及禁止操作清单。
>
> **关联文档**：`01-MCP架构总览.md`、`05-OPC-ethos-server-spec.md`
>
> **最后更新**：2026-05-17
...
`OPC-sandbox` 是 OPC 平台最关键的安全基础设施之一，为 AI 员工（尤其是 DEV 类开发角色）提供受控的代码执行环境。AI 员工生成的代码在此环境中运行、测试和验证，确保学生在不暴露本地环境风险的前提下，获得即时的代码执行反馈。
...
```bash
# 创建 cgroup 并设置资源限制
mkdir -p /sys/fs/cgroup/opc-sandbox/student_abc123

echo "500000 1000000" > /sys/fs/cgroup/opc-sandbox/student_abc123/cpu.max
echo "536870912" > /sys/fs/cgroup/opc-sandbox/student_abc123/memory.max
echo "104857600" > /sys/fs/cgroup/opc-sandbox/student_abc123/memory.swap.max
echo "10" > /sys/fs/cgroup/opc-sandbox/student_abc123/pids.max
```
...
```dockerfile
FROM python:3.11-slim-bookworm AS OPC-sandbox-base
...
network_whitelist:
  ...
  # OPC 平台内部
  internal_domains:
    - "api.OPC.edu.local"        # OPC 内部API
    - "atlas.OPC.edu.local"      # 知识图谱服务
    - "memory.OPC.edu.local"     # 记忆服务
...
```yaml
# OPC-sandbox-security-rules.yml
rules:
...
| 变量名 | 默认值 | 说明 |
|--------|--------|------|
| `OPC_SANDBOX_RUNTIME` | `gvisor` | 沙箱运行时 (gvisor / firecracker / docker) |
| `OPC_SANDBOX_IMAGE` | `OPC-sandbox-base:latest` | 基础镜像 |
| `OPC_SANDBOX_MAX_CONCURRENT` | `20` | 最大并发沙箱数 |
| `OPC_SANDBOX_PORT` | `8083` | MCP Server 端口 |
