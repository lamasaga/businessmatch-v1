...
>
> **蓝图定位**：Phase B 已落盘 **长三角六城** 内容包（`content/world/yangtze_6`）；**运行时 `domains/world/` 表/API 仍自 Phase C+ 落地**。详见 [04-实施路线与蓝图](./04-实施路线与里程碑.md)。
>
> **最后更新**：2026-06-01

**AI 阅读指引（Phase 门控）**：Phase A/B **默认只读** §一～§三 + §十二 纪律 + §十三 索引。§六～§九 为 Phase B+ 数据/POP/AI 深读，**勿**在商赛引擎任务中默认 attach。母本见 `content/world/`（`yangtze_6` 已可用）。
...
　　商识唯智 在 [01-平台愿景](./01-平台愿景与产品架构.md) 主线上，经历 Arena、智能体教练、生涯、图谱、课程等阶段后，形成**两条并列的终局深化路径**：
...
### 5.4 当前工程碎片（Phase B 起步）

| 引擎 | 城市表达 | 统一 ID |
|------|----------|---------|
| trading-v2-rts | **`content/world/yangtze_6`** 六城真实 id（v2.2+） | ✅ `nanjing`/`shanghai`/… |
| trading-v1 | 京城/沪市等虚构简称 + type | 🔴 待迁移 |
| techventure-v1 | 南京/合肥/杭州 + consumers | 🟡 部分真实名，待 `$ref` 母本 |

　　`trading-v2-rts` 已通过 [ADR-010](./docs/decisions/010-拟真城市内容包与CyberCore合并加载.md) 在 CyberCore 加载时合并 world 包；**World 域表/API 仍未实现**。

---

<!-- AI_DEEP:world-data-pop-ai -->

## 六、数据颗粒度：五层模型

> **深读 · Phase B+**：五层颗粒度、POP、数据采集、AI 分工。Phase A 禁止建 `domains/world/` 表/API。
...
　　对齐 [77-商识唯智 AI 作用全景](./inspire/77-商识唯智AI草案.md)（规则优先、LLM 渐进）与 [78-实施指南](./inspire/78-智能体技术路线批判与VibeCoding实施指南.md)（B0 数据真身、C- 最小 LLM）：**拟真城市建设阶段，AI 是生产力与起草器，不是数值真相源。**
...
　　[80-角色 IP 与可成长规则 NPC](./inspire/80-角色IP复用与可成长规则NPC-Agent框架.md) 关注 **Career NPC / 规则 bot 的人格与对话**；拟真城市关注 **地理-经济本体与 POP 基线**。全站 **六支柱 AI 框架**见 [81-](./inspire/81-商识唯智AI赋能六支柱全景.md)。交集仅在于：Athena/Demia 引用 `city_id` 与 `pop_id` 时，须读 World 母本，不得编造与母本矛盾的产业或人口结构。
...
| 项 | 状态 |
|----|------|
| `content/world/cities/_schema.yaml` + ≥6 城母本（含 `pop_segments`） | ✅ `yangtze_6` |
| CyberCore `world_loader` 合并 `region_id` | ✅ [ADR-010](./docs/decisions/010-拟真城市内容包与CyberCore合并加载.md) |
| `trading-v2-rts` 引用真实六城 | ✅ v2.2.0 |
| 新赛制 YAML 从母本引用（禁止内联复制） | 🟡 v1/TV 待迁 |
| POP 行为引擎替换静态 pool | 🔴 `behavior_packs` 占位 |
...
| AI 阶段门控（规则优先） | [inspire/77-](./inspire/77-商识唯智AI草案.md) · [inspire/78-](./inspire/78-智能体技术路线批判与VibeCoding实施指南.md) |
