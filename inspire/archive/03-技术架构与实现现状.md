...
　　**运行中技术栈唯一表**：[`08-` §2.1](./08-工程现状与webapp实现详表.md#21-技术栈摘要)（勿在本节维护副本）。规模化目标见 §1.2。
...
webapp/
├── backend/
│   ├── app/domains/      # ★ arena · career · cybercore · sandbox
...
contracts/                # OpenAPI、域事件、表归属（见 webapp/contracts/）
```
...
| `sandbox` | `domains/sandbox/` + `main.py` | 赛事工坊：热 YAML、内存试跑、publish | 🟡 MVP（无 DB） |

**已挂载路由**（`main.py`）：`/api/v1` → auth, wiki, courses, opc, organizer, **teaching_groups**, competitions, trading, **trading WebSocket**, **practice**, **techventure**, **techventure_admin**, **sandbox**。
...
**赛事分层（引擎 / 配置 ID / match_kind）**：产品侧权威叙述见 [`02-`](./02-赛事体系与双端产品.md) **§5.0**；同一 `engine` 可多份 `game-configs/*.yaml`（练习、国内正式、地区正式赛），`match_kind` 区分办赛流程，结算内核不复制。
...
| OPC | `/opc/*` | 公司、员工、任务 UI |
| 赛事工坊 | `/sandbox` | YAML 热编辑、试跑、调试（内部工具） |
...
　　**唯一权威**：[`08-` §三](./08-工程现状与webapp实现详表.md#三与顶层规划的对齐度) 与 [AI_DEFAULT 快照](./08-工程现状与webapp实现详表.md#ai_default)。本节不维护百分比副本。
...
| mock → Career API | [06-生涯模式](./06-生涯模式-大循环家园与资源经济.md) · [d/03-激励细则](./inspire/d商业模拟教育平台/03-生涯模式与激励闭环.md) |
...
　　P0/P1 完整表见 [08- §2.10](./08-工程现状与webapp实现详表.md#210-已知问题清单工程向)；编码默认见 [AI_DEFAULT](./08-工程现状与webapp实现详表.md#ai_default)。
...
| 路由归属 | 08- AI_DEFAULT + `webapp/contracts/` | 新增路由必须归入正确的 API 模块 |
| 表归属 | 09- §五 | 新建表须声明唯一主人域 |
| 赛制扩展 | `ARCHITECTURE.md` checklist | 新赛制走 YAML + 引擎包，不克隆 `trading.py` |
| 状态/百分比 | 08- §三 | 不得自行编造对齐度，以 08- 为准 |
| 架构「为什么」 | [`docs/decisions/`](./docs/decisions/README.md) ADR 001～008 | 选型争议时 @ 对应 ADR，而非重复争论 |

### 9.2 ADR（跨会话架构记忆）

　　**何时写、如何写**：[`docs/decisions/README.md`](./docs/decisions/README.md) · 自动规则 [`.cursor/rules/adr-writing.mdc`](./.cursor/rules/adr-writing.mdc)。基线决策已收录 001～008（单库、域边界、CyberCore、RTS 单写者、双前端、XP 账本、Phase A）。

### 9.3 AI 编程上下文注入清单

　　**唯一权威任务包表**：[`09-` §6.1](./09-分项目开发与集成流程.md#61-ai-编程上下文注入清单)（含 T0/T1/T2 分层）。本节不重复矩阵。
