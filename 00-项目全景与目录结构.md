...
| **02** | [赛事体系与双端产品](./02-赛事体系与双端产品.md) | Arena、十赛制、CyberCore；§5.0 引擎/配置分层 |
...
| **08** | [工程现状与webapp实现详表](./08-工程现状与webapp实现详表.md) | 路由/API/Bug/百分比**唯一真相源** |
...
**代码**：[`webapp/`](./webapp/) · **OPC 规格库**：`OPC/`（读 `05-` 再下钻）
...
| AI 六支柱（产品框架） | [inspire/81-](./inspire/81-商域AI赋能六支柱全景.md) | 教练/NPC/对手/城市/文书/OPC |
| AI 投资人摘要 | [inspire/77-](./inspire/77-商域AI草案.md) | 现状 vs 愿景 |
...
│   │   └── content/                game-configs · game-assets · world/cities
│   ├── frontend/                   学生端 :5173
│   └── organizer-frontend/         组织者端 :5174
├── OPC/
├── inspire/                        构想库（外链仅由 §1.4 与 50- 维护）
└── PPT/
```
...
> 内容资产：`inspire/a～f/`；历史长文见 `inspire/archive/`、`d商业模拟教育平台/`、`f早期调研/`。
