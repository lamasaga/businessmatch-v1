...
- **API**：`/api/v1` — auth, wiki, courses, opc, organizer, competitions, trading, **trading WebSocket**, **practice**  
- **后端域包**：`domains/{arena,career,cybercore}` · `games/trading`（含 `rts_*` 模块与 `rts_scheduler`）· 赛制 YAML：`trading-v1`（回合制）、**`trading-v2-rts`（浮生记即时）**  
...
- **详表**：见 [08-](./08-工程现状与webapp实现详表.md) §2.11
