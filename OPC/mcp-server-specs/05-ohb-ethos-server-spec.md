# OPC-ethos MCP Server 规格文档

> **文档定位**：定义 OPC 平台伦理审计系统的 MCP Server 完整规格，包含隐私政策合规检查、密钥泄露扫描、内容安全审查、抄袭检测与公平性审计等核心能力。
>
> **关联文档**：`01-MCP架构总览.md`、`04-OPC-sandbox-server-spec.md`
>
> **最后更新**：2026-05-17
...
`OPC-ethos` 是 OPC 平台的伦理与合规守门人。在 AI 员工执行任务的过程中，所有涉及对外发布、数据处理和代码提交的操作都必须经过 Ethos 的审计。它确保学生在使用 AI 工具的过程中遵守法律法规、平台政策和教育伦理，防止隐私泄露、内容违规和学术不端等问题的发生。
...
| 变量名 | 默认值 | 说明 |
|--------|--------|------|
| `OPC_ETHOS_RULES_PATH` | `./ethos-rules` | 规则库路径 |
| `OPC_ETHOS_ML_ENDPOINT` | — | AI审查模型端点 |
| `OPC_ETHOS_PORT` | `8084` | MCP Server 端口 |
