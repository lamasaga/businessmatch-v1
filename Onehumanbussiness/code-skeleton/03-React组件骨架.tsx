...
export const AgencyPage: React.FC = () => {
  ...
  const { employees, fetchEmployees, hireEmployee, currentCompany } = useOHBStore();
...
export const TaskBoard: React.FC<TaskBoardProps> = ({ companyId }) => {
  const { tasks, fetchTasks, createTask } = useOHBStore();
...
export const useOHBStore = create<OHBState>()(
  persist(
    (set, get) => ({
      companies: [],
      employees: [],
      ...
      fetchEmployees: async (companyId: string) => {
        const response = await fetch(`/api/v1/ohb/companies/${companyId}/employees`);
      ...
      fetchTasks: async (companyId: string) => {
        const response = await fetch(`/api/v1/ohb/companies/${companyId}/tasks`);
        const data = await response.json();
        set({ tasks: data.data });
      },
