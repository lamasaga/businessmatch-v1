...
> **文档定位**：OneHumanBusiness (OHB) 模块全部 REST API 端点的 OpenAPI 3.0 风格规格定义，涵盖请求/响应 Schema、认证、限流与错误处理。
>
> **关联文档**：`01-完整数据库Schema.md`、`04-技术栈选型与实现路线.md`
>
> **最后更新**：2026-05-17
>
> **基础 URL**：`https://api.ohb.edu/v1`
> **API 前缀**：`/api/v1/ohb`
...
## 二、公司管理 (/api/v1/ohb/companies/*)

### 2.1 创建公司

```yaml
POST /api/v1/ohb/companies
认证: 必填 (JWT)
速率限制: 写操作级别
...
```yaml
GET /api/v1/ohb/companies/{company_id}
认证: 必填 (JWT)
速率限制: 读操作级别
...
```yaml
PATCH /api/v1/ohb/companies/{company_id}
认证: 必填 (JWT)
速率限制: 写操作级别
...
```yaml
DELETE /api/v1/ohb/companies/{company_id}
认证: 必填 (JWT)
速率限制: 写操作级别
...
```yaml
GET /api/v1/ohb/companies/{company_id}/dashboard
认证: 必填 (JWT)
速率限制: 读操作级别
...
## 三、AI 员工管理 (/api/v1/ohb/employees/*)

### 3.1 获取员工列表

```yaml
GET /api/v1/ohb/companies/{company_id}/employees
认证: 必填 (JWT)
速率限制: 读操作级别
...
```yaml
POST /api/v1/ohb/companies/{company_id}/employees
认证: 必填 (JWT)
速率限制: 写操作级别
...
```yaml
GET /api/v1/ohb/employees/{employee_id}
认证: 必填 (JWT)
速率限制: 读操作级别
...
```yaml
PATCH /api/v1/ohb/employees/{employee_id}
认证: 必填 (JWT)
速率限制: 写操作级别
...
```yaml
POST /api/v1/ohb/employees/{employee_id}/skills/upgrade
认证: 必填 (JWT)
速率限制: 写操作级别
...
```yaml
DELETE /api/v1/ohb/employees/{employee_id}
认证: 必填 (JWT)
速率限制: 写操作级别
...
## 四、任务管理 (/api/v1/ohb/tasks/*)

### 4.1 创建任务

```yaml
POST /api/v1/ohb/companies/{company_id}/tasks
认证: 必填 (JWT)
速率限制: 写操作级别
...
```yaml
POST /api/v1/ohb/tasks/{task_id}/assign
认证: 必填 (JWT)
速率限制: 写操作级别
...
```yaml
GET /api/v1/ohb/companies/{company_id}/tasks
认证: 必填 (JWT)
速率限制: 读操作级别
...
```yaml
PATCH /api/v1/ohb/tasks/{task_id}
认证: 必填 (JWT)
速率限制: 写操作级别
...
```yaml
POST /api/v1/ohb/tasks/{task_id}/review
认证: 必填 (JWT)
速率限制: 写操作级别
...
```yaml
GET /api/v1/ohb/tasks/{task_id}/logs
认证: 必填 (JWT)
速率限制: 读操作级别
...
## 五、流水线管理 (/api/v1/ohb/pipeline/*)

### 5.1 创建 Sprint

```yaml
POST /api/v1/ohb/companies/{company_id}/sprints
认证: 必填 (JWT)
速率限制: 写操作级别
...
```yaml
GET /api/v1/ohb/companies/{company_id}/sprints
认证: 必填 (JWT)
速率限制: 读操作级别
...
```yaml
PATCH /api/v1/ohb/sprints/{sprint_id}
认证: 必填 (JWT)
速率限制: 写操作级别
...
```yaml
POST /api/v1/ohb/companies/{company_id}/gateway-reviews
认证: 必填 (JWT)
速率限制: 写操作级别（限 5次/小时）
...
```yaml
GET /api/v1/ohb/companies/{company_id}/gateway-reviews
认证: 必填 (JWT)
速率限制: 读操作级别
...
```yaml
GET /api/v1/ohb/companies/{company_id}/milestones
认证: 必填 (JWT)
速率限制: 读操作级别
...
## 六、财务管理 (/api/v1/ohb/finance/*)

### 6.1 记账

```yaml
POST /api/v1/ohb/companies/{company_id}/finance/records
认证: 必填 (JWT)
速率限制: 写操作级别
...
```yaml
GET /api/v1/ohb/companies/{company_id}/finance
认证: 必填 (JWT)
速率限制: 读操作级别
...
```yaml
GET /api/v1/ohb/companies/{company_id}/finance/records
认证: 必填 (JWT)
速率限制: 读操作级别
...
## 七、MCP 工具 (/api/v1/ohb/mcp/*)

### 7.1 获取可用 MCP 工具列表

```yaml
GET /api/v1/ohb/companies/{company_id}/mcp/tools
认证: 必填 (JWT)
速率限制: 读操作级别
...
```yaml
POST /api/v1/ohb/companies/{company_id}/mcp/tools/{tool_id}/call
认证: 必填 (JWT)
速率限制: MCP 级别（20次/分钟）
...
```yaml
GET /api/v1/ohb/mcp/calls/{call_id}
认证: 必填 (JWT)
速率限制: 读操作级别
...
```yaml
GET /api/v1/ohb/companies/{company_id}/mcp/calls
认证: 必填 (JWT)
速率限制: 读操作级别
...
## 八、报告生成 (/api/v1/ohb/reports/*)

### 8.1 生成 BMC 报告

```yaml
POST /api/v1/ohb/companies/{company_id}/reports/bmc
认证: 必填 (JWT)
速率限制: 报告级别（5次/小时）
...
```yaml
POST /api/v1/ohb/companies/{company_id}/reports/finance
认证: 必填 (JWT)
速率限制: 报告级别（5次/小时）
...
```yaml
GET /api/v1/ohb/reports/{report_id}
认证: 必填 (JWT)
速率限制: 读操作级别
...
```yaml
POST /api/v1/ohb/employees/{employee_id}/reports/performance
认证: 必填 (JWT)
速率限制: 报告级别（5次/小时）
...
## 九、市场情报 (/api/v1/ohb/intel/*)

### 9.1 获取市场情报列表

```yaml
GET /api/v1/ohb/companies/{company_id}/intel
认证: 必填 (JWT)
速率限制: 读操作级别
...
```yaml
POST /api/v1/ohb/companies/{company_id}/intel
认证: 必填 (JWT)
速率限制: 写操作级别
...
```yaml
POST /api/v1/ohb/companies/{company_id}/intel/collect
认证: 必填 (JWT)
速率限制: 写操作级别（10次/小时）
...
```yaml
PATCH /api/v1/ohb/intel/{intel_id}
认证: 必填 (JWT)
速率限制: 写操作级别
...
```yaml
WS /ws/ohb/companies/{company_id}
...
```yaml
WS /ws/ohb/employees/{employee_id}
认证: JWT Token 通过 query param: ?token=<jwt>
...
| # | 方法 | 端点 | 认证 | 限流类别 |
|---|------|------|------|----------|
| 1 | POST | `/api/v1/ohb/companies` | JWT | 写操作 |
| 2 | GET | `/api/v1/ohb/companies` | JWT | 读操作 |
| 3 | GET | `/api/v1/ohb/companies/{id}` | JWT | 读操作 |
| 4 | PATCH | `/api/v1/ohb/companies/{id}` | JWT | 写操作 |
| 5 | DELETE | `/api/v1/ohb/companies/{id}` | JWT | 写操作 |
| 6 | GET | `/api/v1/ohb/companies/{id}/dashboard` | JWT | 读操作 |
| 7 | GET | `/api/v1/ohb/companies/{id}/employees` | JWT | 读操作 |
| 8 | POST | `/api/v1/ohb/companies/{id}/employees` | JWT | 写操作 |
| 9 | GET | `/api/v1/ohb/employees/{id}` | JWT | 读操作 |
| 10 | PATCH | `/api/v1/ohb/employees/{id}` | JWT | 写操作 |
| 11 | DELETE | `/api/v1/ohb/employees/{id}` | JWT | 写操作 |
| 12 | POST | `/api/v1/ohb/employees/{id}/skills/upgrade` | JWT | 写操作 |
| 13 | POST | `/api/v1/ohb/companies/{id}/tasks` | JWT | 写操作 |
| 14 | GET | `/api/v1/ohb/companies/{id}/tasks` | JWT | 读操作 |
| 15 | GET | `/api/v1/ohb/tasks/{id}` | JWT | 读操作 |
| 16 | PATCH | `/api/v1/ohb/tasks/{id}` | JWT | 写操作 |
| 17 | POST | `/api/v1/ohb/tasks/{id}/assign` | JWT | 写操作 |
| 18 | POST | `/api/v1/ohb/tasks/{id}/review` | JWT | 写操作 |
| 19 | GET | `/api/v1/ohb/tasks/{id}/logs` | JWT | 读操作 |
| 20 | POST | `/api/v1/ohb/companies/{id}/sprints` | JWT | 写操作 |
| 21 | GET | `/api/v1/ohb/companies/{id}/sprints` | JWT | 读操作 |
| 22 | PATCH | `/api/v1/ohb/sprints/{id}` | JWT | 写操作 |
| 23 | POST | `/api/v1/ohb/companies/{id}/gateway-reviews` | JWT | 评审 |
| 24 | GET | `/api/v1/ohb/companies/{id}/gateway-reviews` | JWT | 读操作 |
| 25 | GET | `/api/v1/ohb/companies/{id}/milestones` | JWT | 读操作 |
| 26 | POST | `/api/v1/ohb/companies/{id}/finance/records` | JWT | 写操作 |
| 27 | GET | `/api/v1/ohb/companies/{id}/finance` | JWT | 读操作 |
| 28 | GET | `/api/v1/ohb/companies/{id}/finance/records` | JWT | 读操作 |
| 29 | GET | `/api/v1/ohb/companies/{id}/mcp/tools` | JWT | 读操作 |
| 30 | POST | `/api/v1/ohb/companies/{id}/mcp/tools/{tool}/call` | JWT | MCP |
| 31 | GET | `/api/v1/ohb/mcp/calls/{call_id}` | JWT | 读操作 |
| 32 | GET | `/api/v1/ohb/companies/{id}/mcp/calls` | JWT | 读操作 |
| 33 | POST | `/api/v1/ohb/companies/{id}/reports/bmc` | JWT | 报告 |
| 34 | POST | `/api/v1/ohb/companies/{id}/reports/finance` | JWT | 报告 |
| 35 | GET | `/api/v1/ohb/reports/{report_id}` | JWT | 读操作 |
| 36 | POST | `/api/v1/ohb/employees/{id}/reports/performance` | JWT | 报告 |
| 37 | GET | `/api/v1/ohb/companies/{id}/intel` | JWT | 读操作 |
| 38 | POST | `/api/v1/ohb/companies/{id}/intel` | JWT | 写操作 |
| 39 | POST | `/api/v1/ohb/companies/{id}/intel/collect` | JWT | 写操作 |
| 40 | PATCH | `/api/v1/ohb/intel/{id}` | JWT | 写操作 |
| 41 | WS | `/ws/ohb/companies/{id}` | JWT | - |
| 42 | WS | `/ws/ohb/employees/{id}` | JWT | - |
