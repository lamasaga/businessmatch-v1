...
> **文档定位**：汇总 OneHumanBusiness 平台需要集成的所有第三方 MCP Server 的配置方式、认证方法、教育优惠申请指南及费用估算。
>
...
- **OHB集成方式**：学生在平台内绑定GitHub账号，自动检测教育包状态
...
**OHB 策略**：学生默认使用 Hobby Plan，如需 Pro 功能由学校统一采购团队版。
...
**OHB 策略**：学生在 Test Mode 下完成所有支付功能学习和测试，真实交易由平台统一代收。
...
resend_auth:
  ...
  from_domain: "ohb.edu.local"  # 或学校自定义域名
```
...
**OHB 策略**：每学生分配一个 Free Tier 项目，数据量超限后引导升级或数据清理。
...
figma_auth:
  ...
  oauth:
    ...
    redirect_uri: "https://ohb.edu.local/callback/figma"
```
...
twitter_auth:
  ...
  redirect_uri: "https://ohb.edu.local/callback/twitter"
...
notion_auth:
  ...
  oauth:
    ...
    redirect_uri: "https://ohb.edu.local/callback/notion"
```
