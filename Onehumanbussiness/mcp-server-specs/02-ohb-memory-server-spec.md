# ohb-memory MCP Server 规格文档

> **文档定位**：定义 OneHumanBusiness 平台 AI 员工长期记忆系统的 MCP Server 完整实现规格，包含所有工具的输入输出 Schema、向量存储技术选型、记忆分片策略与隐私隔离机制。
>
> **关联文档**：`01-MCP架构总览.md`、`03-ohb-atlas-server-spec.md`
>
> **最后更新**：2026-05-17
...
`ohb-memory` 是 OHB 平台的核心内部 MCP Server 之一，负责为所有 AI 员工提供长期记忆能力。不同于 LLM 的上下文窗口限制，`ohb-memory` 通过向量数据库存储和检索 AI 员工的历史经验、对话上下文、项目决策与反思，使 AI 员工能够「记住」过往任务、从经验中学习，并在新任务中引用相关知识。
...
def get_collection_name(scope: str, scope_id: str, memory_type: str) -> str:
    ...
    """
    return f"ohb_{scope}_{scope_id}_{memory_type}"

# 示例
collection_name = get_collection_name("employee", "emp_dev01_abc123", "experience")
# 结果: "ohb_employee_emp_dev01_abc123_experience"
```
...
def memory_search(query: str, employee_id: str, company_id: str, 
    ...
    collections = [
        f"ohb_employee_{employee_id}_experience",
        f"ohb_employee_{employee_id}_reflection",
        f"ohb_company_{company_id}_project_context",
    ]
...
| 变量名 | 默认值 | 说明 |
|--------|--------|------|
| `OHB_MEMORY_ENGINE` | `chroma` | 向量存储引擎 (`chroma` / `qdrant`) |
| `OHB_CHROMA_PATH` | `./data/chroma` | ChromaDB 持久化路径 |
| `OHB_QDRANT_HOST` | `localhost` | Qdrant 服务地址 |
| `OHB_QDRANT_PORT` | `6333` | Qdrant HTTP 端口 |
| `OHB_EMBEDDING_MODEL` | `text-embedding-3-small` | 默认嵌入模型 |
| `OHB_OPENAI_API_KEY` | — | OpenAI API Key |
| `OHB_MEMORY_MAX_QUERY_LEN` | `2000` | 最大查询长度 |
| `OHB_MEMORY_DEFAULT_TOPK` | `10` | 默认返回结果数 |

### 8.2 启动方式

```bash
# stdio 模式（MCP 标准）
python -m ohb_memory_server --transport stdio

# SSE 模式（HTTP Server-Sent Events）
python -m ohb_memory_server --transport sse --port 8081
```
