...
| [**00~04**](./00-PROJECT.md) | **第 2 层 · 编写代码时的权威与事实** | 产品定义、工程详表、Phase、域边界、任务→文档链；**03-ENGINEERING.md** 为实现度唯一真相源 |
| [**inspire/**](./inspire/50-目录导航.md) | 构想库（非编程契约） | 方向性长文；**仅由权威文档链入**，文内互链见链接纪律 |

```text
打开项目 → agent.md（导航 + 立场）
              ├─ 写代码 / 查事实 → 00~04（尤其 03-ENGINEERING.md、02-ARCHITECTURE.md）
              ├─ 开 AI 会话 → 03-ENGINEERING.md §开发流程 + .cursor/rules/
              ├─ 跑起来 → webapp/README.md
              └─ 读构想 / 教研 → 00-PROJECT.md §inspire 专题索引 → inspire/50-
```
...
| **弄清仓库结构与选读路径** | [`00-PROJECT.md`](./00-PROJECT.md) |
| **本地启动演示** | [`webapp/README.md`](./webapp/README.md) |
| **体验营 P1（营团+商赛）** | [`01-PRODUCT.md`](./01-PRODUCT.md) §双端产品 · [`03-ENGINEERING.md`](./03-ENGINEERING.md) §AI_DEFAULT · `arena/ARCHITECTURE.md` |
| **读 AI 六支柱 / 教师端构想等** | [`00-PROJECT.md`](./00-PROJECT.md) §inspire 专题索引 → [`inspire/50-`](./inspire/50-目录导航.md) |
...
　　**凡涉及实现、路由、表归属、Phase 能否做**，以 **`00~04` + `docs/decisions/` + 各域 `DESIGN.md`** 为准，**不以 inspire 构想稿为准**。

| 问题 | 唯一 / 首选真相源 |
| ---- | ---------------- |
| 路由、API、Bug、实现度 % | [`03-ENGINEERING.md`](./03-ENGINEERING.md) §AI_DEFAULT |
...
　　完整开场模板、**T0/T1/T2 任务包**、注入矩阵见 **[`03-ENGINEERING.md`](./03-ENGINEERING.md) §开发流程**。每次会话 **2～3 句**声明：做什么 · Phase · 涉及域。

| 层 | 内容 | 预算 |
|----|------|------|
| **T0** | 本文 + `.cursor/rules/` | ≤4k（自动） |
| **T1** | [`03-ENGINEERING.md`](./03-ENGINEERING.md) §AI_DEFAULT + 任务包 1～2 份 | ≤12k |
| **T2** | `03-ENGINEERING.md` 全表、inspire 长文 | 按需 grep/局部 Read |

　　**禁止**默认全文 attach [`03-ENGINEERING.md`](./03-ENGINEERING.md)、inspire 长文。
...
| 规则 | 说明 |
| ---- | ---- |
| **链入 inspire** | **仅** `agent.md`、`README.md`、`00~04` 可维护指向 `inspire/` 的 Markdown 链接；详见 [doc-linking.mdc](./.cursor/rules/doc-linking.mdc) |
| **inspire 文内** | 可链向 `00~04`、`agent.md`、`webapp/`、`docs/decisions/`；**禁止** `inspire/` 之间相互链接（用编号指称，索引进 [`50-`](./inspire/50-目录导航.md)） |
...
1. **先读本文导航**，再打开任务对应的 `00~04` 权威节，不凭印象改代码。  
2. **有歧义时问主理人** Phase 与域，不擅自扩大范围。  
3. **inspire 超前于代码** 时，以 `03-ENGINEERING.md` 与 Phase 为准；构想只作方向参考。  
...
- **产品线 / 新端 / Phase 推进**：更新 [`00-PROJECT.md`](./00-PROJECT.md) 与 [`03-ENGINEERING.md`](./03-ENGINEERING.md)；本文 §二 导航仅在有**新入口级**变化时改。  
- **新增稳定 inspire 专题（50+ 编号）**：登记 [`inspire/50-`](./inspire/50-目录导航.md)，并在 [`00-PROJECT.md`](./00-PROJECT.md) §inspire 专题索引 增加**一条**权威外链。  
- **AI 协作流程变更**：改 [`03-ENGINEERING.md`](./03-ENGINEERING.md) §开发流程，本文 §四 保持链指向即可。
