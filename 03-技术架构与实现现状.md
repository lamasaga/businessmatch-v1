...
│   ├── content/game-configs/   # ★ trading-v1.yaml
│   └── app/api/          # 含 practice 练习场
...
| `cybercore` | `domains/cybercore/` + `content/game-configs/` | 加载赛制 YAML | ✅ `trading-v1`；多赛制待增 |
| `games/trading` | `games/trading/engine.py` | 交易赛结算内核 | ✅ 从 API 抽离；与 YAML defaults 对齐中 |
...
**已挂载路由**（`main.py`）：`/api/v1` → auth, wiki, courses, opc, organizer, competitions, trading, **practice**。
