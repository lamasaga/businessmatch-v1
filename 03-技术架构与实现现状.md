...
```
webapp/
├── backend/              # 平台 API 真身（按域分包）
├── frontend/             # 学生端 → 未来 apps/web-student
├── organizer-frontend/   # ★ 待建：组织者端
├── 00-、01-、README.md
packages/                 # ★ 渐进：api-types, cybercore-engine, game-config-schema
content/game-configs/     # 赛制 YAML
contracts/                # OpenAPI、域事件、表归属约定
```
...
| 域包 | 职责 | 现状 API 文件 |
|------|------|---------------|
| `identity` | 用户、JWT | `auth.py` |
| `arena` | 比赛、交易赛 | `competitions.py`, `trading.py`, `organizer.py` |
| `career` | 生涯、XP 事件 | 分散在用户表/前端 persist 🔴 |
| `academy` | 课程 | `courses.py`（硬编码列表） |
| `atlas` | Wiki、图谱 | `wiki.py` |
| `OPC` | 一人公司 CRUD | `OPC.py` |

**已挂载路由**（`main.py`）：`/api/v1` → auth, wiki, courses, OPC, organizer, competitions, trading。
...
| 组件 | 对齐度 | 缺口摘要 |
|------|--------|----------|
| Career Hub | 🟡 30% | 无 `xp_events` 幂等账本；mock 榜单 |
| Atlas | 🟡 50% | Wiki/图谱有；掌握度驱动解锁弱 |
| Academy | 🟡 40% | UI 有；进度未回写生涯 |
| Quest | 🟡 35% | 前端页；无 streak 服务 |
| Arena | 🟡 45% | 交易赛 official 可玩；practice+AI 未独立 |
| Credenti | 🔴 20% | mock |
| Athena | 🟡 25% | 模板/mock |
| Demia/Rival | 🔴 15% | 演示 |
...
| 优先级 | 问题 |
|--------|------|
| P0 | 交易赛逻辑难复用；无 `match_kind`；组织者未独立端 |
