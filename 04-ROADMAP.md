...
## 二、Phase A：商赛引擎闭环（✅ 2026-06-30 验收通过）
...
| 浮生记 RTS v2 | 游戏日叙事 + 调度器单写 + WS + AI 七人 | ✅ |
...
| Career 前端读账本 | `/career` 对接真实后端：XP + 金币 + 钻石 + 等级 + 近期对局 + 规则复盘 | ✅ |
| 房间码加入 | 幂等加入 + 自动跳转大厅/局内（E3） | ✅ |

**Phase A 退出条件**：上表全部 ✅ 即视为 **Phase A 验收通过**，可启动 Phase B1。

**范围外（非 PA）**：**赛事工坊**（`/sandbox`）——代码已存在 MVP 骨架，但**不计入 Phase A 交付与验收**；教研/引擎可视化编排难度超出 PA 范围，搁置至 **B4 或更后** backlog（实现参考 inspire/89-，日常改赛制仍直接编辑 `content/game-configs/*.yaml`）。
...
| 赛事工坊（可选） | `/sandbox` 热 YAML、试跑、publish、admin 门控；非入校闭环必需 | ⏸️ backlog |

**Phase B 退出条件**：B1~B4 全部验收通过（赛事工坊为可选 backlog，不阻塞 B 退出）。
...
| Phase | 能做 | 不能做 |
|-------|------|--------|
| **A（已验收 2026-06-30）** | 维护与小修 | 抢跑 B2+ 家园/World/OPC Agent；**赛事工坊非 PA 范围** |
| **B1（下一迭代）** | 营内对抗 + 在线匹配 + AI 填位、Game Shell 全引擎统一 | 抢跑 Hermes LLM、教师教研工作台 |
