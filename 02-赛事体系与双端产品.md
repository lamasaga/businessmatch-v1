...
>
> **历史过程稿**：原 `12-赛事体系架构与性格适配` 已重构并入本文；`inspire/archive/` 目录已移除，勿再引用归档副本。
...
> **P1 已落地（2026-05-29）**：独立表 `teaching_groups` + `group_memberships` + `POST/GET /api/v1/teaching-groups/*`；6 位营团邀请码与 4 位商赛房间码分离；学生端 `/camp`、教师端 `:5174` 营团页。**赛季 `seasons`/里程碑仍待建**——见 [ADR-009](./docs/decisions/009-赛季模式与教师端双端演进.md)。
...
| 3 | 确认加入 | `POST /teaching-groups/join`（`invite_code`）→ 写入 `group_memberships` |
