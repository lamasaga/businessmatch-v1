...
**6 轮策略商赛——三城布局、四线择一、三维投入、BQI 评分**：每支队伍经营一款科技产品，在「南京·合肥·杭州」三城中选择战略布局路线（技术驱动 / 用户深耕 / 品牌传播 / 破局奇兵），每轮将预算分配到 Tech（研发）、Fit（用户匹配）、Show（品牌展示）三个维度，最终以**加权累计市场声量**排名决胜负。
...
weight = round_weights[round_no]  # R1=0.08, R2=0.10, R3=0.14, R4=0.18, R5=0.22, R6=0.28
...
| **数据库设计** | 每轮每队一条 `tv_submissions` + `tv_snapshots`，6 轮 × 40 队 = 240 行/表 |
| **并发提交** | 决策提交各自写入，无锁竞争 |
...
- 采用二级菜单经营中控台，避免战略、投入、城市、新闻、排行全部平铺

### 8.1.1 局内信息架构

TECH 局内界面必须遵循“本轮决策优先、反馈与情报分层”的原则。首屏保持顶部 HUD、左侧二级菜单、中间主舞台、右侧态势面板、底部行动栏五个区域。

| 区域 | 责任 | 关键内容 |
|------|------|----------|
| 顶部 HUD | 比赛状态 | 队伍、产品、轮次、预算、总分、倒计时 |
| 左侧二级菜单 | 信息分流 | 战略、投入、市场、情报、排行 |
| 中间主舞台 | 当前任务 | 路线与城市选择、预算投入、三城市场、KPI、排行 |
| 右侧态势面板 | 常驻摘要 | 路线、声量、名次、已开城市、新闻 |
| 底部行动栏 | 本轮提交 | 当前决策摘要、产品宣言、提交按钮 |

禁止把战略选择、预算滑块、KPI、新闻、排行榜同时堆叠在一个长页面里。所有高频操作必须在当前菜单或底部行动栏内完成，鼠标移动路径应短，提交前能直接看到路线、城市、Tech/Fit/Show 和剩余预算。

路线选择卡片只展示路线名称、标签、简短自然语言说明与切换成本；不得在路线卡片内展示 Tech/Fit/Show 权重柱状图，以免学生误把路线加成当作本轮投入结果。投入表单在同一轮内必须保持本地编辑状态，轮询刷新、菜单切换或鼠标误触不得清空 Tech/Fit/Show 与宣言草稿；只有进入新轮次、切换队伍或提交状态变化时才允许重新初始化。

### 8.1.2 视觉规范

TECH 使用独立视觉系统，不复用 FST 配色。主基调为白、蓝、金、灰：白色承载主内容面板，蓝色用于主操作与 TECH 路线，金色用于机会、亮点与 PATHFINDER，灰色用于中性信息、分隔线和 USER 相关辅助表达。紫色、深青绿不得作为 TECH 主色或大面积背景色，只能作为“创想/练习”相关的小面积点缀；不得用于标题主渐变、主进度条、轮次当前状态或关键操作按钮。

白色不得成为唯一视觉主导。首屏应通过顶部/主舞台标题带、预算模块进度、关键行动按钮、路线状态与重要数值，形成足够面积的蓝色与金色视觉锚点。预算结算模块必须使用浅色卡片、蓝色标题/主数值、金色进度或风险提示、灰色分隔与辅助文字；不得使用大面积深灰背景承载财务信息。

### 8.2 组件清单

| 组件 | 路径 | 状态 | 说明 |
|------|------|------|------|
| `TechVenturePlayPage` | `pages/Games/TechVenturePlayPage.tsx` | ✅ 已实现 | 二级菜单经营中控台 |
...
3. **战略页**：选择四路线，开拓三城，主舞台显示三城节点
4. **投入页**：滑块调整 Tech/Fit/Show，预算面板实时显示支出和剩余
5. **市场页**：查看三城 Geek/Pragmatic/Trendy 客群结构与本队 Fit/Show 基础
6. **情报页**：查看 KPI、BQI 因素、历史反馈
7. **排行页**：查看 Top 10 和本队位置
8. **底部行动栏**：持续显示当前决策摘要，输入 ≤60 字产品宣言并提交
9. **等待结算**：轮询 `poll` 查看状态变化
10. **复盘反馈**：新闻、BQI、KPI 和排名变化分别进入对应菜单
...
| KPI 图表 | Tech 蓝色、声量灰蓝、BQI 金色/蓝色辅助 |
| 新闻图标 | 按 kind 分类的图标 |
| 奖杯奖牌 | 结算奖励展示 |
...
meta:
  ...
  description: 6轮策略商赛——三城布局、三维投入、BQI评分
  theme: techventure
  education_focus:
    - 战略路线选择与取舍
    - 资源分配与投入产出
...
| `round_weights` | [0.08, 0.10, 0.14, 0.18, 0.22, 0.28] | 各轮成绩权重（越往后越重要） |
...
| **决策周期** | 6 轮，每轮 8 分钟 | 5 秒 tick，8~20 分钟 | 5 轮 + 特殊事件，60~120 分钟 |
