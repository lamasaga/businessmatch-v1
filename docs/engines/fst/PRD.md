...
### 2.4.1 呈现层：游戏日叙事（产品强制）

　　FST **不得**在玩家可见界面使用「拍」「tick」「节拍」等引擎术语；一律以**日历日期 + 一日将尽**叙事增强代入感。

| 要求 | 说明 |
|------|------|
| HUD | 顶栏「游戏日期」槽内嵌小型日晷/太阳弧（`FstDayClock`），展示当日流逝；**禁止**额外占一整行的大时间条 |
| 指令反馈 | 「指令已排队 · 约 N 秒后执行（今日末统一入账）」 |
| 日末锁 | 约最后 0.6s 提示「日末结算」；禁止新指令 |
| 运输 | 「预计 X **日后**到达」 |
| 入账 toast | 「X月X日 入账」+ 现金/资产变动 |
| 实现映射 | API 仍用 `tick` 等字段；前端 `webapp/frontend/src/lib/fstGameTime.ts` 负责日期换算 |
| 术语登记 | [`00-TERMINOLOGY.md`](../../../00-TERMINOLOGY.md) **游戏日**；引擎细则见 [`GUIDE.md`](./GUIDE.md) §零 |
...
### 2.8 3.3 更新目标：经营类游戏式操作台

3.3 版重点解决“信息平铺、操作割裂、鼠标来回跑”的体验问题。FST 不应让玩家在右侧选商品、左下确认、再回到地图找城市；局内界面应像成熟经营类游戏一样，把“我看到什么、我点哪里、结果在哪里发生”连成一个自然动作。

| 目标 | 设计要求 | 验收口径 |
|------|----------|----------|
| 两级菜单 | 一级固定为地图/市场/车队/分销/订单；二级随选中对象展开 | 玩家不需要在多个远离区域之间来回找按钮 |
| 所见即所得 | 点击城市、商品、路线后，就在对象附近出现可执行动作 | 买卖和移动都从当前关注对象发起 |
| 顺手操作 | 商品卡内完成数量选择与买/卖确认；城市节点内完成移动确认 | 高频操作鼠标移动距离显著缩短 |
| 信息收束 | 主屏只放地图、当前操作区、关键 HUD；详情进入抽屉/浮层 | 避免所有 UI 平铺造成信息噪音 |
| 视觉统一 | common-ui 图标、FST 专属 HUD、角色面板统一尺寸和层级 | 图标、界面、角色形象不会互相抢占空间 |
...
| 设置分销商 | `set_distributor` | `{"city": string, "product_id": string, "side": "buy" \| "sell", "limit_price": number, "quantity": int}` | 在当前城市设置自动买卖委托 |
| 观望 | `hold` | `{}` | 本日不操作 |
...
| 分销商委托 | `set_distributor` | `{"city": string, "product_id": string, "side": "buy" \| "sell", "limit_price": number, "quantity": int}` | 在城市部署有经营成本的分销商，按限价自动收购/卖出 | P0 |
...
- **车辆加成**：
  - `van`：+18 格
  - `truck`：+42 格
- **商品体积**：1~18 不等。民生品体积低、周转快；家具/机械/汽车占用大量仓储，其中 `passenger_car` 体积为 18，用于表达整车运输的高运力门槛。
...
随机事件不得简单、立刻修改某个商品价格。事件必须先作用于城市中某些居民/产业阶层，再改变相关商品的生产量、消费需求或物流能力，最后由供给和需求曲线自然传导到价格。事件用来制造“可预判、可抢跑、可解释”的机会窗口。每个事件分为 3 个阶段：

| 阶段 | 时长 | 玩家可见性 | 作用 |
|------|------|------------|------|
| 预告 | 1~3 日 | 模糊提示，如“上海高端展会传闻” | 给玩家提前囤货/转向的机会 |
| 生效 | 3~10 日 | 明确事件卡、影响商品、城市、居民/产业阶层 | 形成高利润窗口 |
...
| 事件类型 | 作用对象 | 影响方式 | 策略点 |
|----------|----------|----------|--------|
| `harvest_bumper` | 大众居民、农业产区 | 粮食/生鲜 `production_mult` 上升 | 低价扫货，运往消费城 |
| `factory_rush` | 工业阶层、制造城 | 下游产出上升，上游原料/能源需求上升 | 抢原料、提前备货 |
| `energy_squeeze` | 工业阶层 | 能源需求上升，高耗能商品产出下降 | 常州/南通 → 制造城 |
| `auto_fair` | 中产/高收入居民 | 家用车、数码需求上升 | 高价低频大单 |
| `tech_launch` | 中产/高收入居民 | 数码、家电需求曲线右移 | 提前从产地进货 |
| `medicine_alert` | 大众/中产居民 | 医药需求上升 | 南京/上海/杭州短缺窗口 |
| `culture_festival` | 中产/高收入居民 | 文创、服装、奢侈需求上升 | 轻体积高周转套利 |
| `logistics_jam` | 路网/城市到货能力 | travel_days +1，间接加深短缺 | 绕路、提前出发、抢拥堵解除 |

事件发生频率要求：

- 标准 10 分钟局应触发 5~8 次事件，其中 2~3 次是强机会窗口。
- 同一城市同一商品不得连续被同向事件锁死；必须有冷却时间。
- 事件预告必须早于生效 1~4 日，且文案说明“影响产出/需求/物流”，不写“价格立刻上涨”。

#### 5.6.1 分销商：自动买卖的经营化表达

自动买卖不以无成本“脚本”形式出现，而是通过在城市设置分销商实现。分销商是玩家的本地经营网点，帮助玩家按限价自动收购或卖出商品，但需要设置成本、持续经营成本和数量上限。

| 字段 | 说明 |
|------|------|
| `setup_cost` | 设置成本，默认 6000 |
| `upkeep_per_tick` | 每日经营成本，默认 120 |
| `max_per_player` | 每名玩家最多 2 个分销商 |
| `price_band_pct` | UI 推荐价带，围绕当前 ask/bid 给出限价建议 |
| `max_daily_volume` | 单个分销商每日最多自动成交数量 |

操作规则：

1. 玩家必须在当前城市设置分销商。
2. 玩家选择商品、买入/卖出方向、限价、每日数量。
3. 每日 tick 时，调度器按当前城市 ask/bid 自动撮合，成交后进入玩家现金与库存。
4. 自动买入仍受现金与仓储约束；自动卖出仍受库存约束。
5. 分销商成交会写入 tick 事件，用于复盘解释“某城价格被自动收购/抛售影响”。
...
FST 局内页采用“中央地图 + 对象上下文操作 + 两级菜单”的经营类游戏布局，而不是把所有市场、库存、操作按钮平铺在页面上。

| 区域 | 内容 | 交互原则 |
|------|------|----------|
| 顶部 HUD | 日期、阶段、现金、总资产、仓储、倒计时 | 只显示最关键状态，图标优先使用 common-ui / lucide |
| 中央地图 | 城市节点、路线、商队、事件、订单 | 点击城市即打开城市上下文菜单 |
| 左侧一级菜单 | 地图、市场、车队、分销、订单、排行 | 只切换功能域，不承载复杂表单 |
| 右侧二级面板 | 当前选中城市/商品/路线/角色的详情 | 随对象变化，展示可执行动作 |
| 底部命令槽 | 当前动作确认、数量条、快捷键提示 | 买卖/移动确认始终贴近当前动作 |

关键操作流：

1. **买入**：点击城市 → 点击商品 → 商品卡内出现数量条与买入按钮，不再移动到页面另一角确认。
2. **卖出**：点击库存商品 → 可售城市在地图高亮 → 在商品详情或目标城市面板内确认卖出。
3. **移动**：点击当前城市 → 相邻城市高亮 → 点击目标城市出现路费、到达日、预计净利，再确认出发。
4. **分销商**：点击城市 → 进入“分销”二级面板 → 选择商品、买/卖方向、限价、每日数量，确认设置。

AI 角色面板不得蜷缩在角落。练习赛的 3 个 AI 与玩家应以“小尺寸竞争者条/角色槽”出现在地图侧边或排行榜上方，使用既有角色背景中的四个人物形象作为头像/半身背景。角色图宽度建议 56~72px，面板高度 72~96px；不得遮挡地图城市节点、价格面板或命令槽。

图标与素材规范：

- common-ui 负责现金、库存、倒计时、趋势、供给、需求、风险、订单、AI、排名等通用语义。
- fushengji 专属素材负责城市、车辆、商品、事件、长三角地图和角色面板背景。
- 商品图标扩展到 16 种，尺寸建议 28px/40px 两档；列表内不超过 28px，详情页可用 40px。
- UI 图标、角色图、按钮和商品图标必须统一基线和视觉重量，避免角色抢过主要操作。
...
| TradingRound JSON 过大 | `price_snapshot` 含 6 城 × 16 商品 | 约 16KB/行，150 day ≈ 2.4MB/场，可接受 |
...
FST 3.3 标准商品数为 16 种。不是每个城市都必须生产每一种商品；部分商品在某些城市只有需求没有产出，用于形成明显区位差异。

商品数值按价格与体积形成 5 个策略档位：

- **民生低值**：粮食、生鲜、日用品，单价低、体积低，适合新手高周转套利。
- **工业输入**：原材料、能源、化工品，单价中低、体积偏高，受制造城事件影响明显。
- **普通消费/制造**：服装、纺织品、家具、文创、医药，价格与体积差异大，用于练习路线选择。
- **耐用品/设备**：家电、数码产品、机械设备，资金占用高，价格弹性和城市差异更强。
- **重资产/奢侈**：家用车、奢侈品。家用车高价且体积极大，适合大车队低频运输；奢侈品高价低体积但需求更集中、波动更强。

| 上游/基础 | 工业投入 | 中游制造 | 消费品 |
|-----------|----------|----------|--------|
| 粮食 `grain` | 原材料 `raw_materials` | 家具 `furniture` | 日用品 `daily_goods` |
| 生鲜 `produce` | 能源 `energy` | 纺织品 `textile` | 家电 `appliances` |
| — | 化工品 `chemicals` | 服装 `apparel` | 数码产品 `digital_device` |
| — | — | 机械设备 `machinery` | 家用车 `passenger_car` |
| — | — | — | 医药 `medicine` / 文创 `cultural_goods` / 奢侈品 `luxury` |
...
- [ ] 经营类游戏式两级菜单：对象上下文操作、底部命令槽、所见即所得确认（Phase A 收尾）
- [ ] common-ui 图标接入：现金、库存、供需、趋势、订单、风险、AI、排名（Phase A 收尾）
- [ ] AI 角色面板：使用四人物形象，重做尺寸与地图/面板关系（Phase A 收尾）
- [ ] 供需状态可视化：短缺/平衡/过剩、供需槽、趋势标签（Phase A 收尾）
- [ ] 商品详情价格曲线：ask/bid、供需指数、事件标记（Phase A 收尾）
- [ ] 路线净利预估：扣除路费后的预计利润与风险解释（Phase A 收尾）
- [x] 城市角色配置：六城产业身份、优势供给、高需求商品、16 品类覆盖（Phase A 收尾）
- [x] 分销商引擎基础：`set_distributor`、经营成本、限价自动买卖（Phase A 收尾）
