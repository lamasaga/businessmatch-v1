...
class OneCompany(Base):
    ...
    __table_args__ = (
        Index("idx_ohb_companies_user", "user_id", "stage"),
    )


class AIEmployee(Base):
    """AI数字员工"""
    __tablename__ = "ohb_employees"

    id = Column(Integer, primary_key=True, index=True)
    company_id = Column(Integer, ForeignKey("ohb_companies.id"), nullable=False)
    ...
    __table_args__ = (
        ...
        Index("idx_ohb_employees_company", "company_id", "status"),
    )


class AITask(Base):
    """AI任务"""
    __tablename__ = "ohb_tasks"

    id = Column(Integer, primary_key=True, index=True)
    company_id = Column(Integer, ForeignKey("ohb_companies.id"), nullable=False)
    assignee_id = Column(Integer, ForeignKey("ohb_employees.id"), nullable=False)
    ...
    __table_args__ = (
        Index("idx_ohb_tasks_company_status", "company_id", "status"),
        Index("idx_ohb_tasks_assignee", "assignee_id", "status"),
        CheckConstraint("student_rating >= 1 AND student_rating <= 5", name="chk_ohb_rating_range"),
    )
