...
import { ArrowLeft, Plus, Star, Briefcase, Search, Filter } from 'lucide-react';
...
export default function TalentMarketPage() {
  ...
  const { company, employees, hireEmployee, fetchEmployees } = useOHBStore();
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400">雇佣 AI 员工，组建你的创业团队</p>
        </div>
      </div>
          ...
          <div className="flex flex-wrap gap-3">
            {employees.map((emp) => (
              ...
              >
                ...
                <span className="text-sm font-medium text-blue-800">{emp.codename}</span>
                <span className="text-xs text-blue-600">Lv.{emp.level}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900">{role.codename}</h3>
            ...
            <div className="flex flex-wrap gap-2 mb-5">
              {role.skills.map((skill) => (
                <span
                  key={skill}
                  className="text-xs px-2 py-1 rounded-md bg-gray-50 text-gray-600 border border-gray-100"
                >
                  {skill}
                </span>
              ))}
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium transition-colors disabled:opacity-50 bg-blue-600 text-white hover:bg-blue-700"
            >
