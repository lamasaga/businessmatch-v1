...
const mockBadges = [
  ...
  { id: '2', name: '策略大师', icon: 'Trophy', description: '赢得3场商赛', earnedAt: '2026-04-15' },
  { id: '3', name: '勤奋学子', icon: 'BookOpen', description: '完成5门课程', earnedAt: '2026-05-01' },
];

const mockRecentGames = [
  { id: '1', name: '回合制策略商赛 #42', type: '回合制', result: '第2名', date: '2026-05-10' },
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    document.title = '个人中心 - 商业模拟平台';
  }, []);

  if (!user) {
    return (
      <div className="text-center py-20">
        <p className="text-foreground-muted">请先登录</p>
    ...
    <div className="space-y-8 animate-fade-in-up">
      ...
      <div className="glass-card p-8">
        <div className="flex items-center gap-6">
          <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-primary to-purple-500 flex items-center justify-center">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 mt-2">
              <span className="px-3 py-1 rounded-full bg-primary-soft text-primary text-sm font-medium capitalize">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-6">
          ...
          <div className="space-y-3">
            {mockBadges.map((badge) => (
              <div
                key={badge.id}
                className="flex items-center gap-4 p-4 rounded-lg bg-background-hover/50"
              >
                ...
                <span className="text-xs text-foreground-muted">{badge.earnedAt}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="glass-card p-6">
          ...
          <Link
            to="/games"
            className="block mt-4 text-center py-2.5 rounded-lg border border-border-subtle text-foreground-secondary hover:bg-background-hover transition-colors text-sm"
          >
            查看更多对局
          </Link>
        </div>
